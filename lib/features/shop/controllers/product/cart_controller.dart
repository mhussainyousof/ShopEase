...
void removeOneFromCart(CartItemModel item) {
    int index = cartItems.indexWhere((carlItem) => carlItem.productId == item.productId && carlItem.variationId == item.variationId);
...
void updateAlreadyAddedProductCount(ProductModel product) {
  // If product has no variations then calculate cart quantities and display total number.
  // Else make default entries to 0 and show cart quantities when variation is selected.
...
void updateCartTotals() {
    ...
    int calculatedMobfItems = 0;

    for (var item in cartItems) {
        calculatedTotalPrice += (item.price) * item.quantity.toDouble();
        calculatedMobfItems += item.quantity;
    }

    totalCartPrice.value = calculatedTotalPrice;
    noOfCartItems.value = calculatedMobfItems;
}
